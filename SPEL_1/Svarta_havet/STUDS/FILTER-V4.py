...
def summarize(shortlist, number_games):
    ...
    for key, value in number_dict.items():
        ...
        for num in value:
            print(num)
            probsum_per_order =+ num
            print(probsum_per_order)
        probsum += probsum_per_order
        print(probsum)

    print("Probability sum: " + str(probsum))
    '''
    
    for key, value in number_dict.items():
        print("Nyckel/order: " + key)
        print("Summa sannolikhet för order: " + str(number_dict[key]))
        if len(number_dict[key]) == 1:
            probsumstring = str(number_dict[key][0])
        else:
            raise ValueError("KNAAAAAAAS!")
        
        print("Summa sannolikhet för order fast string: " + probsumstring)
        probsum += float(probsumstring)
        print("probsum: " + str(probsum))
        
    print("Slutgiltig probability sum: " + str(probsum))
    
    # Divide each number in the dictionary by the divisor
    for key, value in number_dict.items():
        if isinstance(key, str):  # Ensuring the key is a string
            number_dict[key] = [num / probsum for num in value]
        else:
            print("KNAS!"*100)
...
info = find_lines_with_phrase(file_path, phrase)
...
document_filename = "FV4-Sitcheck_" + country_string + "_" + info[4]+ "-" + info[5]+ "-" + info[6] + ".txt"
