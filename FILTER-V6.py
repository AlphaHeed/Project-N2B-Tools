# This code reads a file "log" containg the log that gets printed when running the task situation-check (several iterations).
# The output file created contains: sitsheck config, Specific power and the suggested actions with probability weights, mean value for all actions, normalised mean for all actions, standard deviation. 
    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if "C:\KEX" in line: # Used to label the output file. Paste the JSON-file adress in the log file. "C:\KEX" is part of all JSON addresses in my case.
                    line = line.strip()
                    info = line.split("\\")
                    # print(info)
                        
    except FileNotFoundError:
        print("File "+ str(file_path) +" not found.")
